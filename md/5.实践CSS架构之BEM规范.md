...
### 通过 TS 类型体操生成 BEM规范样式

E和M可以是数组，可以通过下标来将数组或对象转化为联合类型
~~~ts
// 数组
T[number]
// 对象
...
在TypeScript中，当`数组`类型为`never`时，表示该数组不包含任何元素。而字符串中有一个为never类型，整个就被判定为never类型。
...
在play测试模块目录下的 src 目录中的 App.vue 文件中把上面写的测试组件进行引入：
...
最后`pnpm run dev`运行，发现类名和css已经符合BEM规范。
